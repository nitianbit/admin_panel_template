...
import { ServicesImages } from "../../types/servicesImages";
...
const AddServicesImagesDialog = ({ isModalOpen, toggleModal, selectedId }: any) => {
    const { onCreate, detail, onUpdate } = useServicesImagesStore();
    const { globalCompanyId } = useCompanyStore();
    const [data, setData] = React.useState<ServicesImages>({
        company: "",
        image: null,
        name: "",
    ...
    const { register, handleSubmit, formState: { errors }, reset } = useForm<ServicesImages>();
    ...
    const onSubmit = async () => {
        ...
        if (data?._id) {
         ...
         if (response?._id && data?.image && typeof data.image === 'object') {
            const res = await uploadFile({ module: 'servicesImages', record_id: response?._id }, [data?.image])
        ...
        <>
            ...
            >
                <form onSubmit={handleSubmit(onSubmit)}>
                    <DialogTitle>Add Services Images</DialogTitle>
...
export default AddServicesImagesDialog