...
import { GalleryImage } from "../../types/galleryImages";
...
const AddGalleryImageDialog = ({ isModalOpen, toggleModal, selectedId }: any) => {
    const { onCreate, detail, onUpdate } = useGalleryImageStore();
    const [data, setData] = React.useState<GalleryImage>({
        image: null,
        name: ""
    })
    const { register, handleSubmit, formState: { errors }, reset } = useForm<GalleryImage>();
    ...
    const onSubmit = async () => {
        ...
        if (response?._id && data?.image && typeof data.image === 'object') {
            const res = await uploadFile({ module: MODULES.GALLERY_IMAGES, record_id: response?._id }, [data?.image])
...
export default AddGalleryImageDialog