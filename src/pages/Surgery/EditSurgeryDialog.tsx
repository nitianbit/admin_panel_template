...
const EditSurgeryDialog = ({ isModalOpen, toggleModal, selectedId }: any) => {
    ...
    const fetchSurgery = async (id: string) => {
        try {
            const res = await doGET(`${ENDPOINTS.detail("surgery", id)}`);
            if (res.data?.data) {
    ...
    useEffect(() => {
        ...
        if (selectedId && isModalOpen) fetchSurgery(selectedId);
    ...
    const handleFileUpload = (event: any) => {
        ...
        setFiles((prev: FileItem[]) => [...prev, ...formatted]);
    };
    ...
    const handleSubmit = async () => {
        try {
            const { full_name, email, concern, speciality } = data;

            let response = null;

            if (data?._id) {
                response = await doPUT(ENDPOINTS.update("surgery"), {
                    _id: data?._id,
                    full_name,
                    email,
                    concern,
                ...
                response = await doPOST(ENDPOINTS.create("surgery"), {
                    full_name,
                    email,
                    concern,
                    speciality
            ...
            if (response.status >= 200 && response.status < 400 && files.length) {
                const uploadRes = await uploadFile(
                    { module: "surgery", record_id: response?.data?.data?._id },
                    files.map((f) => f.file)
                );

                if (uploadRes.status >= 200 && uploadRes.status < 400) {
                    await doPUT(ENDPOINTS.update("surgery"), {
                    ...
                    setFiles([]);
                    fetchSurgery(response?.data?.data?._id);
                }
            }
    ...
    return (
        <>
            ...
            >
                <DialogTitle>Surgery Record</DialogTitle>
                            ...
                            {data?.medical_record?.length ? (
                                <Box sx={{ mt: 2 }}>
                                    {data.medical_record.map((file: string, index: number) => (
                                        <a
                                            key={index}
                                            href={file} // already prefixed with FILE_BASE_URL
                                            target="_blank"
