...
import { EventState, Event, EventFilters } from '../../types/events';




const store = create<EventState>((set, get) => ({
    data: [],
    totalPages: 0,
    currentPage: 1,
    filters: {},
    ...
    fetchGrid: async () => {
        try {
            ...
            const apiUrl = `${ENDPOINTS.grid('event')}?${queryParams.toString()}`;
    ...
    fetchGridAll: async (filters: EventFilters) => {
        try {
            const { allData } = get();
    ...
    onCreate: async (data: any) => {
        try {
            const response = await doPOST(ENDPOINTS.create('event'), data);
    ...
    onUpdate: async (data: Event) => {
        try {
            const response = await doPUT(ENDPOINTS.update('event'), data);
            console.log(response);
            if (response.status >= 200 && response.status < 400) {
                get().fetchGrid();
            }
    ...
    onDelete: async (id: string) => {
        try {
            const response = await doDELETE(ENDPOINTS.delete('event', id));
            console.log(response);
            if (response.status >= 200 && response.status < 400) {
                get().fetchGrid();
            }
    ...
    detail: async (id: string) => {
        try {
            const response = await doGET(ENDPOINTS.detail('event', id));
...
export const useEventStore = () => store((state) => state);
