...
import { Banner, BannerFilters, BannerState } from '../../types/banners';
...
const store = create<BannerState>((set, get) => ({
    ...
    fetchGrid: async () => {
        try {
            ...
            const apiUrl = `${ENDPOINTS.grid(MODULES.BANNER)}?${queryParams.toString()}`;
    ...
    onCreate: async (data: any) => {
        try {
            const response = await doPOST(ENDPOINTS.create(MODULES.BANNER), data);
    ...
    onUpdate: async (data: Banner) => {
        try {
            const response = await doPUT(ENDPOINTS.update(MODULES.BANNER), data);
            console.log(response);
            if (response.status >= 200 && response.status < 400) {
                get().fetchGrid();
            }
    ...
    onDelete: async (id: string) => {
        try {
            const response = await doDELETE(ENDPOINTS.delete(MODULES.BANNER, id));
            console.log(response);
            if (response.status >= 200 && response.status < 400) {
                get().fetchGrid();
            }
    ...
    detail: async (id: string) => {
        try {
            const response = await doGET(ENDPOINTS.detail(MODULES.BANNER, id));
