...
import {
  ...
  EwaPackageFilters,
  EwaPackageState
} from '../../types/ewaPackage';

const store = create<EwaPackageState>((set, get) => ({
  data: [],
  totalPages: 0,
  total: 0,
  currentPage: 1,
  ...
  fetchGrid: async () => {
    ...
    try {
      ...
      const apiUrl = `${ENDPOINTS.grid('ewa-package')}?${queryParams.toString()}`;
      ...
      showError('Failed to fetch Ewa Packages');
    } finally {
      set({ isLoading: false });
    }
  },

  setFilters: (newFilters: EwaPackageFilters) => {
...
export const useEwaPackageStore = () => store((state) => state);
